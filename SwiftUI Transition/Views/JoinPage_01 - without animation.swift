...
struct JoinPage_01_Previews: PreviewProvider {
    struct HostView: View {
        @State private var showSheet = true

        var body: some View {
            Button("Sign up") { showSheet.toggle() }
                .sheet(isPresented: $showSheet) { JoinPage_01() }
        }
    }
    ...
    static var previews: some View {
        Group {
            views.previewDisplayName("No Transition")
        }
    }
}
