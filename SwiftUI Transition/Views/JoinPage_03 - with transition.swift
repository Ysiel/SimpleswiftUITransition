...
extension JoinPage_03: View {
    var body: some View {
        VStack {
            switch viewState {
            case .welcome:
                welcomeView.transition(.opacity.animation(.default))
            case .waiting:
                waitingView.transition(.opacity.animation(.default))
            case .success:
                successView.transition(.opacity.animation(.default))
            }
        }
...
struct JoinPage_03_Previews: PreviewProvider {
    struct HostView: View {
        @State private var showSheet = true

        var body: some View {
            Button("Sign up") { showSheet.toggle() }
                .sheet(isPresented: $showSheet) { JoinPage_03() }
        }
    }
    ...
    static var previews: some View {
        Group {
            views.previewDisplayName("Default Transition")
        }
    }
}
