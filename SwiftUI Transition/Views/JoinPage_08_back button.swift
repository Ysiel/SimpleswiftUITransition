...
struct JoinPage_08 {
    ...
    @State private var confirmationTansition: AnyTransition = .moveInFromRightOutToLeft
}

extension JoinPage_08: View {
    var body: some View {
        VStack {
            switch viewState {
            case .welcome:
            ...
            case .confirmation:
                confirmationView.transition(confirmationTansition)
            case .waiting:
    ...
    private func backToWelcome() async {
        confirmationTansition = .moveInOutFromRight_v2
        withAnimation {
            viewState = .welcome
        }
    }
    ...
    private func proceed() async {
        confirmationTansition = .moveInFromRightOutToLeft
        withAnimation { viewState = .waiting }
        try? await pauseTask(1)
        withAnimation(.blobby) { viewState = .success }
    }
}

extension JoinPage_08 {
...
struct JoinPage_08_Previews: PreviewProvider {
    struct HostView: View {
        @State private var showSheet = true

        var body: some View {
            Button("Sign up") { showSheet.toggle() }
                .sheet(isPresented: $showSheet) { JoinPage_08() }
        }
    }

    @ViewBuilder static var views: some View { HostView() }

    static var previews: some View { views.previewDisplayName("With Back Button") }
}
